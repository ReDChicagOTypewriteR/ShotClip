...
#ifdef SHOTCLIP_OFFLINE
    updateModelsButton->hide();
    auto *offlineNotice = new QLabel(
        tr("ShotClip offline mode: choose an existing local executable and GGML model. "
           "Model downloads are disabled."),
        m_configWidget);
    offlineNotice->setWordWrap(true);
    configLayout->addWidget(offlineNotice, 3, 0, 1, 3);
#endif
...
void TranscribeAudioDialog::refreshModels(bool report)
{
    ...
    QMessageBox::information(
        this,
        tr("Offline Mode"),
        tr("ShotClip does not download models. Select a local GGML model using Browse."));
    return;
#else
