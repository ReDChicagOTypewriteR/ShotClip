...
class RippleDeleteSubtitlesCommand : public QUndoCommand
{
public:
    RippleDeleteSubtitlesCommand(SubtitlesModel &model,
                                 int trackIndex,
                                 int64_t startMs,
                                 int64_t endMs);
    void redo();
    void undo();
    bool changesItems() const { return m_changes; }

private:
    SubtitlesModel &m_model;
    int m_trackIndex;
    QList<SubtitleItem> m_before;
    QList<SubtitleItem> m_after;
