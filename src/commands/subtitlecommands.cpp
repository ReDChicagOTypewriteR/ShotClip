...
RippleDeleteSubtitlesCommand::RippleDeleteSubtitlesCommand(SubtitlesModel &model,
                                                           int trackIndex,
                                                           int64_t startMs,
                                                           int64_t endMs)
    : QUndoCommand(0)
    ...
    , m_trackIndex(trackIndex)
{
    const int count = m_model.itemCount(trackIndex);
    m_before.reserve(count);
    for (int i = 0; i < count; ++i)
        m_before << m_model.getItem(trackIndex, i);
    m_after = rippleDeletedItems(m_before, startMs, endMs);
    m_changes = m_before.size() != m_after.size();
    if (!m_changes) {
        for (int i = 0; i < m_before.size(); ++i) {
            if (m_before.at(i).start != m_after.at(i).start
                || m_before.at(i).end != m_after.at(i).end
                || m_before.at(i).text != m_after.at(i).text) {
                m_changes = true;
                break;
            }
        }
    }
    setText(QObject::tr("Ripple delete subtitles"));
}

void RippleDeleteSubtitlesCommand::redo()
{
    m_model.doReplaceSubtitleItems(m_trackIndex, m_after);
}

void RippleDeleteSubtitlesCommand::undo()
{
    m_model.doReplaceSubtitleItems(m_trackIndex, m_before);
}
