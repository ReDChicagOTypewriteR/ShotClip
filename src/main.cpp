...
#include <winbase.h>
        ...
        CreateMutexA(NULL, FALSE, "AleXJokeR ShotClip Running Mutex");
#else
        dir.cdUp();
#endif
#ifdef Q_OS_MAC
        ...
        setOrganizationName("AleXJokeR");
#if defined(Q_OS_UNIX) && !defined(Q_OS_MAC)
        setOrganizationDomain("alexjoker.top");
        setDesktopFileName("top.alexjoker.ShotClip");
#else
        setOrganizationDomain("alexjoker.top");
#endif
        setApplicationName("ShotClip");
        setApplicationDisplayName("ShotClip");
        setApplicationVersion(SHOTCLIP_VERSION);
        setWindowIcon(QIcon(QStringLiteral(":/shotclip/shotclip-mark.svg")));
...
#endif
        // ShotClip is designed for fully offline environments. The option remains accepted for
        // compatibility, but automatic update checks are always disabled in this fork.
        setProperty("noupgrade", true);
        ...
        if (!dir.exists())
            dir.mkpath(dir.path());
        const auto logFileName = dir.filePath("shotclip-log.txt");
        if (QFile::exists(logFileName)) {
            const auto previousLogName = dir.filePath("shotclip-log.bak");
...
int main(int argc, char **argv)
{
    ...
    if (::qEnvironmentVariableIsSet(kWatchdogEnvVar)) {
        ...
        // Log some basic info.
        LOG_INFO() << "Starting ShotClip version" << SHOTCLIP_VERSION << "based on Shotcut"
                   << SHOTCUT_VERSION;
#if defined(Q_OS_WIN)
