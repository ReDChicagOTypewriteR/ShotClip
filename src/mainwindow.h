...
signals:
    ...
    void autosaveCompleted(bool success, const QString &path);
...
private:
    ...
    void refreshAgentContext(bool includeStructuredContext = true);
    QJsonObject buildAgentTimelineContext();
    void applyAgentOperations(const QList<AgentEditOperation> &operations,
                              const QString &contextId);
    bool removeTimelineRange(int startFrame, int endFrame);
