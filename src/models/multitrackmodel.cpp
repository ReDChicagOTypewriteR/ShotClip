...
int MultitrackModel::ensureShotClipIds()
{
    if (!m_tractor || !m_tractor->is_valid())
        return 0;

    int assigned = 0;
    const auto newId = []() { return QUuid::createUuid().toString(QUuid::WithoutBraces); };
    if (QString::fromUtf8(m_tractor->get(kShotClipProjectIdProperty)).isEmpty()) {
        m_tractor->set(kShotClipProjectIdProperty, newId().toUtf8().constData());
        ++assigned;
    }
    m_tractor->set(kShotClipSchemaVersionProperty, 1);

    QSet<QString> trackIds;
    QSet<QString> clipIds;
    QHash<int, QString> groupLinkIds;
    for (int trackIndex = 0; trackIndex < m_trackList.size(); ++trackIndex) {
        QScopedPointer<Mlt::Producer> track(m_tractor->track(m_trackList.at(trackIndex).mlt_index));
        if (!track || !track->is_valid())
            continue;

        QString trackId = QString::fromUtf8(track->get(kShotClipTrackIdProperty));
        if (trackId.isEmpty() || trackIds.contains(trackId)) {
            trackId = newId();
            track->set(kShotClipTrackIdProperty, trackId.toUtf8().constData());
            ++assigned;
        }
        ...
        for (int clipIndex = 0; clipIndex < playlist.count(); ++clipIndex) {
            ...
            QString clipId = QString::fromUtf8(info->cut->get(kShotClipClipIdProperty));
            if (clipId.isEmpty() || clipIds.contains(clipId)) {
                clipId = newId();
                info->cut->set(kShotClipClipIdProperty, clipId.toUtf8().constData());
                ++assigned;
            }
            clipIds.insert(clipId);
            MLT.ensureHasUuid(*info->cut);

            QString linkId = QString::fromUtf8(info->cut->get(kShotClipLinkIdProperty));
            if (info->cut->property_exists(kShotcutGroupProperty)) {
                const int group = info->cut->get_int(kShotcutGroupProperty);
                if (!linkId.isEmpty() && !groupLinkIds.contains(group))
                    groupLinkIds.insert(group, linkId);
                if (groupLinkIds.contains(group)) {
                    linkId = groupLinkIds.value(group);
                } else {
                    linkId = newId();
                    groupLinkIds.insert(group, linkId);
                }
            ...
            if (QString::fromUtf8(info->cut->get(kShotClipLinkIdProperty)) != linkId) {
                info->cut->set(kShotClipLinkIdProperty, linkId.toUtf8().constData());
                ++assigned;
            }
        }
    }
...
QString MultitrackModel::shotClipTrackId(int trackIndex) const
{
    if (!m_tractor || trackIndex < 0 || trackIndex >= m_trackList.size())
        return QString();
    QScopedPointer<Mlt::Producer> track(m_tractor->track(m_trackList.at(trackIndex).mlt_index));
    return track ? QString::fromUtf8(track->get(kShotClipTrackIdProperty)) : QString();
}

QString MultitrackModel::shotClipClipId(int trackIndex, int clipIndex) const
{
    if (!m_tractor || trackIndex < 0 || trackIndex >= m_trackList.size())
        return QString();
    QScopedPointer<Mlt::Producer> track(m_tractor->track(m_trackList.at(trackIndex).mlt_index));
    ...
    Mlt::Playlist playlist(*track);
    QScopedPointer<Mlt::ClipInfo> info(playlist.clip_info(clipIndex));
    return info && info->cut ? QString::fromUtf8(info->cut->get(kShotClipClipIdProperty))
                             : QString();
}

bool MultitrackModel::rippleDeleteRange(int position, int length)
{
    if (!m_tractor || position < 0 || length <= 0)
        return false;
    for (int trackIndex = 0; trackIndex < m_trackList.size(); ++trackIndex) {
        QScopedPointer<Mlt::Producer> track(m_tractor->track(m_trackList.at(trackIndex).mlt_index));
        if (!track || track->get_int(kTrackLockProperty))
            return false;
    }
    for (int trackIndex = 0; trackIndex < m_trackList.size(); ++trackIndex)
        removeRegion(trackIndex, position, length);
