...
void SubtitlesModel::doReplaceSubtitleItems(
    int trackIndex, const QList<Subtitles::SubtitleItem> &subtitles)
{
    if (trackIndex < 0 || trackIndex >= m_tracks.size()) {
        LOG_ERROR() << "Invalid index" << trackIndex;
        return;
    ...
    beginResetModel();
    m_items[trackIndex] = subtitles;
    requestFeedCommit(trackIndex);
    endResetModel();
}

void SubtitlesModel::rippleDeleteRange(int64_t startMs, int64_t endMs)
{
    if (startMs < 0 || endMs <= startMs)
        return;
    for (int trackIndex = 0; trackIndex < m_tracks.size(); ++trackIndex) {
        auto *command = new Subtitles::RippleDeleteSubtitlesCommand(
            *this, trackIndex, startMs, endMs);
        if (command->changesItems())
            MAIN.undoStack()->push(command);
        else
            delete command;
    }
