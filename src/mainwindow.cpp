...
#include "Logger.h"
#include "actions.h"
#include "agent/timelinesafetycommand.h"
    ...
    , m_keyframesDock(0)
{
    ...
    // Create the UI.
    ui->setupUi(this);
    ui->actionAbout_Shotcut->setText(tr("About ShotClip"));
...
void MainWindow::setupAndConnectDocks()
{
    ...
    connect(m_timelineDock->model(), &MultitrackModel::modified, this, [this]() {
        m_timelineDock->model()->ensureShotClipIds();
    });
    connect(m_timelineDock->model(),
    ...
    ui->mainToolBar->insertAction(ui->dummyAction, m_agentDock->toggleViewAction());
    connect(m_agentDock, &AgentDock::seekRequested, this, [this](double seconds) {
        seekTimeline(qRound(seconds * MLT.profile().fps()));
    });
    connect(m_agentDock, &AgentDock::undoRequested, this, [this]() {
        if (m_undoStack->canUndo()) {
            m_undoStack->undo();
            m_agentDock->applyFinished(true, tr("已撤销上一次时间线修改。"));
        } else {
            m_agentDock->applyFinished(false, tr("当前没有可以撤销的时间线修改。"));
        }
    });
    connect(m_agentDock,
            &AgentDock::applyRequested,
            this,
            &MainWindow::applyAgentOperations);
    connect(m_timelineDock, &TimelineDock::positionChanged, this, [this]() {
        refreshAgentContext(false);
    });
    connect(m_timelineDock, &TimelineDock::durationChanged, this, [this]() {
        refreshAgentContext();
    });
    connect(m_timelineDock->model(), &MultitrackModel::modified, this, [this]() {
        refreshAgentContext();
    });
    connect(m_timelineDock->subtitlesModel(), &SubtitlesModel::modified, this, [this]() {
        refreshAgentContext();
    });
    ...
    connect(m_agentDock, &AgentDock::showSubtitlesRequested, this, [this]() {
        m_subtitlesDock->show();
        m_subtitlesDock->raise();
    });
    connect(m_agentDock, &AgentDock::transcribeRequested, this, [this]() {
        if (!multitrack() || !multitrack()->is_valid()
            || m_timelineDock->model()->trackList().isEmpty()) {
            m_agentDock->applyFinished(false, tr("当前时间线为空，无法开始本地转录。"));
            return;
        }
        m_subtitlesDock->show();
        m_subtitlesDock->raise();
        m_subtitlesDock->startSpeechToText();
        m_agentDock->refreshLocalModelStatus();
    });
    connect(m_agentDock, &AgentDock::recoveryPointRequested, this, [this]() { doAutosave(); });
    connect(this,
            &MainWindow::autosaveCompleted,
            m_agentDock,
            &AgentDock::setRecoveryStatus);
    ...
    tabifyDockWidget(m_elementsDock, m_agentDock);
    // Bottom area
    ...
    // Keep the editing canvas spacious on first launch. The Agent is one shortcut away and
    // QMainWindow restores the user's docked/floating/visible state on subsequent launches.
    m_agentDock->hide();
    resetDockCorners();
    refreshAgentContext();
}
...
void MainWindow::doAutosave()
{
    ...
    emit autosaveCompleted(success, path);
}
...
void MainWindow::on_actionAbout_Shotcut_triggered()
{
    ...
    QMessageBox::about(
        ...
        QStringLiteral(
            "<h1>%1 %2 Demo</h1>"
            "<p>面向完全离线长视频粗剪工作流的本地 AI 剪辑实验版本。</p>"
            "<p>作者：<a href=\"http://alexjoker.top/\">AleXJokeR</a></p>"
            "<p>Based on <a href=\"https://www.shotcut.org/\">Shotcut %3</a> and the MLT "
            "multimedia framework.</p>"
            "<small><p>%4</p>"
            "<p>Licensed under the %5. ShotClip modifications and the complete corresponding "
            "source code must remain available when distributed.</p>"
            ...
            .arg(qApp->applicationName(),
                 ...
                 QStringLiteral(SHOTCUT_VERSION),
                 copyright,
                 license));
}
...
void MainWindow::refreshAgentContext(bool includeStructuredContext)
{
    if (!m_agentDock || !m_timelineDock)
        return;
    auto *tractor = m_timelineDock->model()->tractor();
    const double fps = MLT.profile().fps();
    const bool hasTimeline = tractor && tractor->is_valid()
                             && !m_timelineDock->model()->trackList().isEmpty()
                             && tractor->get_length() > 0 && fps > 0.0;
    const double duration = hasTimeline ? tractor->get_length() / fps : 0.0;
    const double position = hasTimeline ? qMax(0, m_timelineDock->position()) / fps : 0.0;
    const QJsonObject structuredContext = includeStructuredContext && hasTimeline
                                              ? buildAgentTimelineContext()
                                              : QJsonObject();
    m_agentDock->setTimelineContext(duration, position, hasTimeline, structuredContext);
}

QJsonObject MainWindow::buildAgentTimelineContext()
{
    QJsonObject context;
    if (!m_timelineDock || !m_timelineDock->model()->tractor()
        || !m_timelineDock->model()->tractor()->is_valid()) {
        return context;
    }
    auto *model = m_timelineDock->model();
    model->ensureShotClipIds();
    const double fps = MLT.profile().fps();
    if (fps <= 0.0)
        return context;

    context.insert(QStringLiteral("version"), QStringLiteral("1.0"));
    context.insert(QStringLiteral("project_id"),
                   QString::fromUtf8(model->tractor()->get(kShotClipProjectIdProperty)));
    context.insert(QStringLiteral("duration_seconds"), model->tractor()->get_length() / fps);
    context.insert(QStringLiteral("fps"), fps);
    context.insert(QStringLiteral("privacy"),
                   QStringLiteral("local_only_no_media_paths"));
    context.insert(QStringLiteral("host_context_tools"),
                   QJsonArray{QStringLiteral("project.get_metadata"),
                              QStringLiteral("timeline.list_tracks"),
                              QStringLiteral("timeline.list_clips"),
                              QStringLiteral("subtitles.list_segments")});

    QJsonArray tracks;
    const auto trackList = model->trackList();
    for (int trackIndex = 0; trackIndex < trackList.size(); ++trackIndex) {
        QJsonObject track;
        track.insert(QStringLiteral("track_id"), model->shotClipTrackId(trackIndex));
        track.insert(QStringLiteral("name"), model->getTrackName(trackIndex).left(200));
        track.insert(QStringLiteral("type"),
                     trackList.at(trackIndex).type == AudioTrackType ? QStringLiteral("audio")
                                                                     : QStringLiteral("video"));
        track.insert(QStringLiteral("locked"), m_timelineDock->isTrackLocked(trackIndex));
        QJsonArray clips;
        const QModelIndex trackModelIndex = model->index(trackIndex);
        const int clipCount = model->rowCount(trackModelIndex);
        for (int clipIndex = 0; clipIndex < clipCount; ++clipIndex) {
            const auto info = model->getClipInfo(trackIndex, clipIndex);
            if (!info || !info->cut || !info->producer || !info->producer->is_valid()
                || info->producer->is_blank()) {
                continue;
            }
            QJsonObject clip;
            clip.insert(QStringLiteral("clip_id"), model->shotClipClipId(trackIndex, clipIndex));
            clip.insert(QStringLiteral("link_id"),
                        QString::fromUtf8(info->cut->get(kShotClipLinkIdProperty)));
            clip.insert(QStringLiteral("timeline_start_seconds"), info->start / fps);
            clip.insert(QStringLiteral("timeline_end_seconds"),
                        (info->start + info->frame_count) / fps);
            clip.insert(QStringLiteral("source_in_seconds"), info->frame_in / fps);
            clip.insert(QStringLiteral("source_out_seconds"), info->frame_out / fps);
            clip.insert(QStringLiteral("kind"),
    ...
    context.insert(QStringLiteral("tracks"), tracks);

    QJsonArray subtitleTracks;
    auto *subtitles = m_timelineDock->subtitlesModel();
    int includedSubtitleItems = 0;
    int totalSubtitleItems = 0;
    int remainingTextBudget = 24000;
    bool subtitleTruncated = false;
    for (int trackIndex = 0; trackIndex < subtitles->trackCount(); ++trackIndex) {
        const auto subtitleTrack = subtitles->getTrack(trackIndex);
        QJsonObject track;
        track.insert(QStringLiteral("name"), subtitleTrack.name.left(200));
        track.insert(QStringLiteral("language"), subtitleTrack.lang.left(40));
        QJsonArray items;
        const int itemCount = subtitles->itemCount(trackIndex);
        totalSubtitleItems += itemCount;
        for (int itemIndex = 0; itemIndex < itemCount; ++itemIndex) {
            const auto item = subtitles->getItem(trackIndex, itemIndex);
            const QString text = QString::fromStdString(item.text).left(240);
            if (remainingTextBudget - text.size() < 0 || includedSubtitleItems >= 600) {
                subtitleTruncated = true;
                break;
            }
            QJsonObject subtitle;
            subtitle.insert(QStringLiteral("start_seconds"), item.start / 1000.0);
            subtitle.insert(QStringLiteral("end_seconds"), item.end / 1000.0);
            subtitle.insert(QStringLiteral("text"), text);
            items.append(subtitle);
            remainingTextBudget -= text.size();
            ++includedSubtitleItems;
        }
        track.insert(QStringLiteral("segments"), items);
        subtitleTracks.append(track);
    }
    context.insert(QStringLiteral("subtitle_tracks"), subtitleTracks);
    context.insert(QStringLiteral("subtitle_items_total"), totalSubtitleItems);
    context.insert(QStringLiteral("subtitle_items_included"), includedSubtitleItems);
    context.insert(QStringLiteral("subtitle_truncated"), subtitleTruncated);

    const QByteArray canonical = QJsonDocument(context).toJson(QJsonDocument::Compact);
    const QString contextId = QString::fromLatin1(
        QCryptographicHash::hash(canonical, QCryptographicHash::Sha256).toHex().left(32));
    context.insert(QStringLiteral("context_id"), contextId);
    context.insert(QStringLiteral("playhead_seconds"),
                   qMax(0, m_timelineDock->position()) / fps);
    return context;
}

void MainWindow::applyAgentOperations(const QList<AgentEditOperation> &operations,
                                      const QString &contextId)
{
    if (!m_agentDock || !m_timelineDock || !m_timelineDock->model()->tractor()
        || m_timelineDock->model()->trackList().isEmpty()) {
        m_agentDock->applyFinished(false, tr("当前时间线为空，请先将素材拖入时间线。"));
        return;
    }

    const QString currentContextId = buildAgentTimelineContext()
                                         .value(QStringLiteral("context_id"))
                                         .toString();
    if (contextId.isEmpty() || contextId != currentContextId) {
        m_agentDock->applyFinished(
            false,
            tr("时间线在方案生成后发生了变化。旧方案已拒绝，请重新分析。"));
        refreshAgentContext();
        return;
    }
    ...
    for (int trackIndex = 0; trackIndex < model->trackList().size(); ++trackIndex) {
        if (m_timelineDock->isTrackLocked(trackIndex)) {
            m_agentDock->applyFinished(
                false,
                tr("轨道 %1 已锁定。为避免音画错位，跨轨编辑已在修改前取消。")
                    .arg(trackIndex + 1));
            return;
        }
    }
    ...
    const double fps = MLT.profile().fps();
    const int timelineFrames = m_timelineDock->model()->tractor()->get_length();
    if (fps <= 0.0 || timelineFrames <= 0) {
        m_agentDock->applyFinished(false, tr("无法读取时间线帧率或时长。"));
        return;
    }
    ...
    for (const auto &operation : operations) {
        if (!operation.executable)
            continue;
        const int start = qBound(0, qRound(operation.startSeconds * fps), timelineFrames);
        const int end = qBound(0, qRound(operation.endSeconds * fps), timelineFrames);
        if (start >= end) {
            m_agentDock->applyFinished(false, tr("建议中存在无效时间范围，时间线未修改。"));
            return;
        }
        if (operation.type == AgentOperationType::RemoveRange) {
            ranges << qMakePair(start, end);
        } else if (operation.type == AgentOperationType::KeepRange) {
            if (end < timelineFrames)
                ranges << qMakePair(end, timelineFrames);
            if (start > 0)
                ranges << qMakePair(0, start);
        }
    }
    if (ranges.isEmpty()) {
        m_agentDock->applyFinished(false, tr("没有可应用到时间线的规则操作。"));
        return;
    }
    ...
    m_undoStack->beginMacro(tr("ShotClip Agent 应用 %1 项建议").arg(operations.size()));
    bool success = true;
    for (const auto &range : merged) {
        if (!removeTimelineRange(range.first, range.second)) {
            success = false;
            break;
        }
    }
    ...
    if (!success) {
        ...
        m_agentDock->applyFinished(
            false,
            tr("跨轨编辑未能完整执行，已回滚全部时间线、字幕和标记修改。"));
        return;
    }
    ...
    m_agentDock->applyFinished(
        true,
        tr("已将 %1 项建议安全应用到全部轨道、字幕和标记，并合并为一次撤销操作。")
            .arg(operations.size()));
}

bool MainWindow::removeTimelineRange(int startFrame, int endFrame)
{
    auto *model = m_timelineDock->model();
    if (!model->tractor() || startFrame < 0 || startFrame >= endFrame)
        return false;

    const int timelineFrames = model->tractor()->get_length();
    endFrame = qMin(endFrame, timelineFrames);

    endFrame = qMin(endFrame, timelineFrames);
    auto *command = new TimelineSafetyCommand(
        *model, *m_timelineDock->markersModel(), startFrame, endFrame - startFrame);
    ...
    const double fps = MLT.profile().fps();
    if (fps > 0.0) {
        const int64_t startMs = qRound64(startFrame * 1000.0 / fps);
        const int64_t endMs = qRound64(endFrame * 1000.0 / fps);
        m_timelineDock->subtitlesModel()->rippleDeleteRange(startMs, endMs);
    }
    return true;
}
